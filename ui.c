...
static void update_words_areas(SDL_Renderer* renderer, Game* game, GameScreen* g_screen, ScreenContext* context) {
	...
	//user`s words
	//НЕЕЕЕТ, ТУТ ДОЛЖНО БЫТЬ ВМЕСТО WORDS_AREA_HEIGHT сделать line_height * MAX_WORDS / 2,
	//ТАКЖЕ НУЖНО ДОБАВИТЬ ЧТО-ТО ТИПА MAX_STRING_WIDTH, И ФУНКЦИЮ, КОТОРАЯ БУДЕТ ПРОБЕГАТЬСЯ ПО WORDS[] И ЗАМЕНЯТЬ ПОСЛЕДНИЕ БУКВЫ НА ..., ЕСЛИ ОНИ НЕ ПОМЕЩАЮТСЯ
